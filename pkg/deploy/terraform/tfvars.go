...
// Deprecated: Use generateCloudInitInDir instead for multi-machine support.
...
func (d *Deployer) generateCloudInitInDir(opts *deploy.DeployOptions, machineDir string) (string, error) {
...
// Deprecated: Use writeTFVarsInDir instead for multi-machine support.
...
// writeTFVarsInDir generates the terraform.tfvars file in the specified directory.
func (d *Deployer) writeTFVarsInDir(opts *deploy.DeployOptions, machineDir, cloudInitPath string) error {
	tfOpts := opts.Terraform
	...
	sb.WriteString(fmt.Sprintf("running      = true\n")) // Always start as running
	sb.WriteString("\n")
	...
	// Cloud-init file - use relative path from machine directory
	relCloudInitPath, err := filepath.Rel(machineDir, cloudInitPath)
	if err != nil {
		relCloudInitPath = cloudInitPath // Fall back to absolute path
	}
	sb.WriteString("# Cloud-Init Configuration\n")
