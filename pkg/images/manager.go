...
// GetISOs returns all registered ISOs.
func (m *Manager) GetISOs() ([]settings.ISO, error) {
	s, err := m.store.Load()
	if err != nil {
		return nil, fmt.Errorf("failed to load settings: %w", err)
	}
...
// AddExistingISO adds an existing ISO file to the registry.
func (m *Manager) AddExistingISO(path, name, version string) (*settings.ISO, error) {
	// Verify file exists
	_, err := os.Stat(path)
	if err != nil {
		return nil, fmt.Errorf("file not found: %w", err)
	...
	// Generate ID
	id := fmt.Sprintf("iso-%s-%d", version, time.Now().Unix())
...
// RemoveISO removes an ISO from the registry.
func (m *Manager) RemoveISO(id string, deleteFile bool) error {
	// Load current settings
	s, err := m.store.Load()
	if err != nil {
		return fmt.Errorf("failed to load settings: %w", err)
	...
	iso := s.FindISO(id)
	if iso == nil {
		return fmt.Errorf("ISO not found: %s", id)
	}

	// Optionally delete file
	if deleteFile && iso.Path != "" {
