...
check_stale_shell_configs() {
    local shell_config_dir="${HOME}/.config/shell"
    local stale_files=(
        "70-starship.sh"   # Renamed to 900-starship.sh
        "75-zoxide.sh"     # Removed from generate-shell-config.sh (now in zoxide.sh)
        "40-podman.sh"     # Podman package removed
        # Note: 80-fzf.sh is now generated by fzf.sh package script, not generate-shell-config.sh
        # but uses the same filename so no stale file issue
    )
    local found_stale=()
    ...
    if [[ ${#found_stale[@]} -gt 0 ]]; then
        echo ""
        log_warning "Found stale shell config files that can be removed:"
        for file in "${found_stale[@]}"; do
            echo "  - ${shell_config_dir}/${file}"
        done
        echo ""
...
main() {
    ...
    # Check for stale shell config files
    check_stale_shell_configs
